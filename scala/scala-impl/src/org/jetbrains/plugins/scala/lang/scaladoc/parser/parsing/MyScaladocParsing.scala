...
class MyScaladocParsing(private val psiBuilder: PsiBuilder) extends ScalaDocElementTypes {
  ...
  private def parseWikiSyntax(implicit builder: PsiBuilder): Boolean = {
    val tokenType = builder.getTokenType match {
    ...
    while (!isEndOfComment) {
      val isNewLine = builder.getTokenType == DOC_WHITESPACE && builder.getTokenText.contains("\n")
      val isLeadingAsterisks = builder.getTokenType == DOC_COMMENT_LEADING_ASTERISKS
      ...
      builder.getTokenType match {
        ...
        case DOC_WHITESPACE if tokenType != DOC_MONOSPACE_TAG =>
          if (/*false && */tokenType == DOC_LINK_TAG) {
